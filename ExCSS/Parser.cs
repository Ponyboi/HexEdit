...
namespace ExCSS
{
    ...
    public sealed partial class Parser
    {
        ...
        public StyleSheet Parse(string css)
        {
            ...
            //lineNumberAppend(css, _styleSheet);


            return _styleSheet;
        }
        public StyleSheet Parse(string css, string fileName)
        {
        ...
        public void lineNumberAppend(string css, StyleSheet styleSheet)
        {
            string[] lines = css.Split(new string[] { "\r\n", "\n" }, StringSplitOptions.None);
            bool checkDeclarations = false;
            int currentStyle = 0;
            int currentDeclaration = 0;
            int lineIndex = 0;
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                var currentStyleSel = styleSheet.StyleRules[currentStyle];
                var currentLineSel = lines[i];
                ...
                {
                    while (styleSheet.StyleRules.Count > currentStyle && lines[i].IndexOf(styleSheet.StyleRules[currentStyle].Value, lineIndex) != -1) 
                    {
                        currentStyleSel = styleSheet.StyleRules[currentStyle];

                        styleSheet.StyleRules[currentStyle].LineNumber = i;
                        lineIndex = lines[i].IndexOf(styleSheet.StyleRules[currentStyle].Value, lineIndex) + styleSheet.StyleRules[currentStyle].Value.Length;
                        checkDeclarations = true;
                        var declarations = styleSheet.StyleRules[currentStyle].Declarations.Properties;

                        while (checkDeclarations && declarations.Count > currentDeclaration && lines[i].IndexOf(declarations[currentDeclaration].Name) != -1)
                            {
                                declarations[currentDeclaration].LineNumber = i;
                                lineIndex = lines[i].IndexOf(declarations[currentDeclaration].Name, lineIndex) + declarations[currentDeclaration].Name.Length;
                                currentDeclaration++;

                                if (styleSheet.StyleRules[currentStyle].Declarations.Count <= currentDeclaration)
                                {
